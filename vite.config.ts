...
interface LangContent { languageName: string; }
    ...
    plugins: [
        ...
        externals({ react: "React", "react-dom": "ReactDOM" }),
        {
            ...
            transformIndexHtml(html) {
                return { html, tags: [presets.react, presets["react-dom"]].map(tag).map(htmlTag) };
            },
        },
        sw_plugin(),
    ],
...
function htmlTag(meta: ReturnType<typeof tag>): HtmlTagDescriptor {
    ...
    return { tag: "script", attrs: { src: url, integrity, crossorigin: "anonymous" }, injectTo: "head" };
}
