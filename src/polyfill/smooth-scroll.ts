export default ((): void => {
    ...
    const ease = (k: number): number => 0.5 * (1 - Math.cos(Math.PI * k));
    interface IContext { method: (x: number, y: number) => void; startTime: number; startX: number; startY: number; stopY: number; }
    const rafID = { current: 0 };
    const cancelScroll = (): void => { cancelAnimationFrame(rafID.current); cleanEventListener(); };
    ...
    const step = (context: IContext): void => {
        ...
        if (elapsed >= 1) { context.method(startX, stopY); cleanEventListener(); return; }
        const currentY = startY + (stopY - startY) * ease(elapsed);
        context.method(startX, currentY);
        if (currentY !== stopY) rafID.current = requestAnimationFrame(() => step(context));
    };
    ...
    Element.prototype.scrollIntoView = function(): void {
        ...
        atachEventListener();
        step({ method: (x: number, y: number) => window.scroll(x, y), startTime: getNow.now(), startX, startY, stopY });
    };
})();
