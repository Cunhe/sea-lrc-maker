...
export const enum ThemeMode { auto, light, dark }
...
const reducer = (state: State, action: Action): State => {
    ...
    return { ...state, [action.type]: typeof payload === "function" ? payload(state) : payload };
};
...
const init = (lazyInit: () => string): State => {
    ...
    state.lang = languages.map((langCode) => {
    ...
    try {
        ...
        for (const key of validKeys) {
            if (key in storedState) (state[key] as unknown) = storedState[key];
        }
    } catch {}
    return state;
};
