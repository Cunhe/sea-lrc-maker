...
type InitArgs = Readonly<{ text: string; options: TrimOptios; select: number }>;

export const enum ActionType { parse, refresh, next, time, info, select, deleteTime, getState }
...
const mergeObject = <T extends O, O>(target: T, obj: O): T => {
    for (const i in obj) {
        if (target[i] !== obj[i]) return { ...target, ...obj };
    }
    return target;
};
    ...
    switch (action.type) {
        ...
        case ActionType.refresh: {
            ...
            const record = state.lyric.reduce((p, c, i) => {
                if (c.time) {
                    if (c.time < p.nextTime && c.time > audioTime) { p.nextTime = c.time; p.nextIndex = i; }
                    if (c.time > p.currentTime && c.time <= audioTime) { p.currentTime = c.time; p.currentIndex = i; }
                }
                return p;
            }, { currentTime: -Infinity, currentIndex: -Infinity, nextTime: Infinity, nextIndex: Infinity });
            return mergeObject(state, record);
        }
        case ActionType.next: {
            const selectIndex = guard(state.selectIndex + 1, 0, state.lyric.length - 1);
            return { ...reducer(state, { type: ActionType.time, payload: action.payload }), selectIndex };
        }
        ...
        case ActionType.info: {
            ...
            if (value.trim() === "") info.delete(name); else info.set(name, value.trim());
            return { ...state, info };
        }
        ...
        case ActionType.deleteTime: {
            ...
            if (lyric[selectIndex].time !== undefined) {
                ...
                if (selectIndex === currentIndex) { currentTime = Infinity; nextTime = -Infinity; }
                return { ...state, lyric, currentTime, nextTime };
            }
            return state;
        }
...
const init = (lazyInit: () => InitArgs): IState => {
    ...
    return { ...parser(text, options), currentTime: Infinity, currentIndex: Infinity, nextTime: -Infinity, nextIndex: -Infinity, selectIndex: select };
};

export const useLrc = (lazyInit: () => InitArgs): [IState, React.Dispatch<Action>] => useReducer(reducer, lazyInit, init);
