...
export const Eidtor: React.FC<{ lrcState: LrcState; lrcDispatch: React.Dispatch<LrcAction>; }> = ({ lrcState, lrcDispatch }) => {
    const { prefState, lang, trimOptions } = useContext(appContext);
    const parse = useCallback((ev: React.FocusEvent<HTMLTextAreaElement>) => {
        lrcDispatch({ type: LrcActionType.parse, payload: { text: ev.target.value, options: trimOptions } });
    }, [lrcDispatch, trimOptions]);
    const setInfo = useCallback((ev: React.FocusEvent<HTMLInputElement>) => {
        const { name, value } = ev.target;
        lrcDispatch({ type: LrcActionType.info, payload: { name, value } });
    }, [lrcDispatch]);
    ...
    const detailsOpened = useMemo(() => sessionStorage.getItem(SSK.editorDetailsOpen) !== "false", []);
    const textarea = useRef<HTMLInputLikeElement>(null);
    const [href, setHref] = useState<string | undefined>(undefined);
    const onDownloadClick = useCallback(() => {
        setHref((url) => {
            if (url) URL.revokeObjectURL(url);
            return URL.createObjectURL(new Blob([textarea.current!.value], { type: "text/plain;charset=UTF-8" }));
        });
    }, []);
    ...
    const canSaveToGist = useMemo(() => localStorage.getItem(LSK.token) !== null && localStorage.getItem(LSK.gistId) !== null, []);
        ...
        setTimeout(() => {
            ...
            if (name) {
                createFile(name, textarea.current!.value).catch((error: Error) => {
                    toastPubSub.pub({ type: "warning", text: error.message });
                });
            }
        }, 500);
    }, [downloadName, lang]);
        ...
        <div className="app-editor">
            <details ref={details} open={detailsOpened} onToggle={onDetailsToggle}>
                ...
                <section className="app-editor-infobox" onBlur={setInfo}>
                    ...
                    <input id="info-ti" name="ti" placeholder={lang.editor.title} {...disableCheck} {...useDefaultValue(lrcState.info.get("ti") || "")} />
                    <label htmlFor="info-ti">]</label>
                    <label htmlFor="info-ar">[ar:</label>
                    <input id="info-ar" name="ar" placeholder={lang.editor.artist} {...disableCheck} {...useDefaultValue(lrcState.info.get("ar") || "")} />
                    <label htmlFor="info-ar">]</label>
                    <label htmlFor="info-al">[al:</label>
                    <input id="info-al" name="al" placeholder={lang.editor.album} {...disableCheck} {...useDefaultValue(lrcState.info.get("al") || "")} />
            ...
            <section className="editor-tools">
                ...
                <button className="editor-tools-item ripple" title={lang.editor.copyText} onClick={onCopyClick}><CopySVG /></button>
                <a className="editor-tools-item ripple" title={lang.editor.downloadText} href={href} onClick={onDownloadClick} download={downloadName}><DownloadSVG /></a>
                <a title={lang.editor.saveToGist} href={canSaveToGist ? undefined : prependHash(ROUTER.gist)} className="editor-tools-item ripple" onClick={canSaveToGist ? onGistSave : undefined}><CloudUploadSVG /></a>
                <a title={lang.editor.utils} href="/lrc-utils/" className="editor-tools-item ripple"><UtilitySVG /></a>
            </section>
            <textarea className="app-textarea" aria-label="lrc input here" onBlur={parse} {...disableCheck} {...useDefaultValue(text, textarea)} />
        </div>
    );
};
