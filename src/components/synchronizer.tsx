...
const SpaceButton: React.FC<{ sync: () => void }> = ({ sync }) => (
    <button className="space-button" onClick={sync}>space</button>
);

export const enum SyncMode { select, highlight }

interface ISynchronizerProps { state: IState; dispatch: React.Dispatch<Action>; }
...
export const Synchronizer: React.FC<ISynchronizerProps> = ({ state, dispatch }) => {
    ...
    useEffect(() => {
        dispatch({
            ...
            payload: { name: "tool", value: `${lang.app.name} https://github.com/Cunhe/sea-lrc-maker` },
        });
    }, [dispatch, lang]);
    ...
    useEffect(() => { sessionStorage.setItem(SSK.syncMode, syncMode.toString()); }, [syncMode]);

    const ul = useRef<HTMLUListElement>(null);
    const needScrollLine = { [SyncMode.select]: selectIndex, [SyncMode.highlight]: highlightIndex }[syncMode];
    useEffect(() => {
        const line = ul.current?.children[needScrollLine];
        if (line !== undefined) line.scrollIntoView({ behavior: "smooth", block: "center", inline: "center" });
    }, [needScrollLine]);

    useEffect(() => currentTimePubSub.sub(self.current, (time) => {
        dispatch({ type: ActionType.refresh, payload: time });
    }), [dispatch]);

    const sync = useCallback(() => {
        if (!audioRef.duration) return;
        dispatch({ type: ActionType.next, payload: audioRef.currentTime });
    }, [dispatch]);

    const adjust = useCallback((ev: KeyboardEvent | React.MouseEvent, offset: number, index: number) => {
        if (!audioRef.duration) return;
        const selectTime = lyric[index]?.time;
        if (selectTime === undefined) return;
        dispatch({ type: ActionType.time, payload: audioRef.step(ev, offset, selectTime) });
    }, [dispatch, lyric]);

    useEffect(() => {
        function onKeydown(ev: KeyboardEvent): void {
            if (isKeyboardElement(ev.target)) return;
            const action = getMatchedAction(ev, keyBindings);
            switch (action) {
                case InputAction.Sync: ev.preventDefault(); sync(); break;
                case InputAction.DeleteTime: ev.preventDefault(); dispatch({ type: ActionType.deleteTime, payload: undefined }); break;
                case InputAction.ResetOffset: ev.preventDefault(); adjust(ev, 0, selectIndex); break;
                case InputAction.DecreaseOffset: ev.preventDefault(); adjust(ev, -0.5, selectIndex); break;
                case InputAction.IncreaseOffset: ev.preventDefault(); adjust(ev, 0.5, selectIndex); break;
                case InputAction.PrevLine: ev.preventDefault(); dispatch({ type: ActionType.select, payload: (index) => index - 1 }); break;
                case InputAction.NextLine: ev.preventDefault(); dispatch({ type: ActionType.select, payload: (index) => index + 1 }); break;
                case InputAction.FirstLine: ev.preventDefault(); dispatch({ type: ActionType.select, payload: () => 0 }); break;
                case InputAction.LastLine: ev.preventDefault(); dispatch({ type: ActionType.select, payload: () => Infinity }); break;
                case InputAction.PageUp: ev.preventDefault(); dispatch({ type: ActionType.select, payload: (index) => index - 10 }); break;
                case InputAction.PageDown: ev.preventDefault(); dispatch({ type: ActionType.select, payload: (index) => index + 10 }); break;
            }
        }
    ...
    const LyricLineIter = useCallback((line: Readonly<ILyric>, index: number, lines: readonly ILyric[]) => {
        ...
        const className = Object.entries({ line: true, select, highlight, error })
            .reduce<string[]>((p, [name, value]) => { if (value) p.push(name); return p; }, [])
            .join(STRINGS.space);
        return <LyricLine key={index} index={index} className={className} line={line} select={select} prefState={prefState} />;
...
interface ILyricLineProps { line: ILyric; index: number; select: boolean; className: string; prefState: PrefState; }
