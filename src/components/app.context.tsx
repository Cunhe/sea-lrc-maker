...
const enum Bits { lang, builtInAudio, prefState }
...
export const appContext = createContext<IAppContext>(undefined, (prev, next) => {
    let bits = 0;
    if (prev.lang !== next.lang) bits |= ChangBits.lang;
    if (prev.prefState.builtInAudio !== next.prefState.builtInAudio) bits |= ChangBits.builtInAudio;
    if (prev.prefState !== next.prefState) bits |= ChangBits.prefState;
    return bits;
});
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const value = useMemo(() => ({
        lang, prefState, prefDispatch,
        trimOptions: { trimStart: prefState.spaceStart >= 0, trimEnd: prefState.spaceEnd >= 0 },
