...
export const audioRef: IAudioRef = {
    ...
    get paused() { return this.current?.paused ?? true; },
    get playbackRate() { return this.current?.playbackRate ?? 1; },
    set playbackRate(rate: number) { if (this.current !== null) this.current.playbackRate = rate; },
    get currentTime() { return this.current?.currentTime ?? 0; },
    set currentTime(time: number) {
        if (this.current !== null && this.current.duration !== 0) this.current.currentTime = time;
    },
    step(ev, value, target): number {
        if (target === undefined) target = this.currentTime;
...
export const enum AudioActionType { pause, getDuration, rateChange }

export type AudioState =
    | { type: AudioActionType.pause; payload: boolean }
    | { type: AudioActionType.getDuration; payload: number }
    | { type: AudioActionType.rateChange; payload: number };
