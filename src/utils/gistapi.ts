...
export const getRepos = async (): Promise<IGistRepo[]> => {
    ...
    const res = await fetch(apiUrl, { method: "GET", headers: { Authorization: `token ${token}` }, mode: "cors" });
...
export const createRepo = async (): Promise<IGistRepo> => {
    ...
    const res = await fetch(apiUrl, {
        ...
        body: JSON.stringify({
            ...
            public: true,
            files: { [GISTINFO.fileName]: { content: GISTINFO.fileContent } },
        }),
    });
    if (!res.ok) throw new Error(res.statusText);
    return res.json();
...
export const getFils = async (): Promise<IGistRepo | null> => {
    ...
    const res = await fetch(`${apiUrl}/${id}`, {
        headers: { Authorization: `token ${token}`, "If-None-Match": etag },
    });
    if (!res.ok && res.status >= 400) throw await res.json();
    localStorage.setItem(LSK.gistEtag, res.headers.get("etag")!);
    sessionStorage.setItem(SSK.ratelimit, JSON.stringify({
