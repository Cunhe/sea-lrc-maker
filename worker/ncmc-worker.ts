...
const AES_ECB_DECRYPT = async (keyData: Uint8Array, data: Uint8Array): Promise<Uint8Array> => {
    const AES = { name: "AES-CBC", iv: new Uint8Array(16) };
    ...
    for (let curser = 0; curser < bodyLength; curser += 16) {
        ...
        const encryptedPadding = await crypto.subtle.encrypt({ name: "AES-CBC", iv: clip }, cryptoKey, padding);
        cipher.set(clip, 0);
...
ncmcWorker.addEventListener("message", async (ev) => {
    ...
    const decryptedData = ((): Uint8Array => {
        ...
        for (let cur = 0; cur < data.length; ++cur) data[cur] ^= keyBox[cur & 0xff];
        return data;
    })();
